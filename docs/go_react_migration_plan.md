...
DTO повторно сгенерированы из OpenAPI; `git diff --check` также прошёл.
Локальная browser-проверка на `http://localhost:4031/profiles/react` подтвердила
загрузку каталога, поиск с URL `q`, открытие профиля по URL `profile` и закрытие
диалога клавишей Escape с возвратом фокуса. Временный стенд остановлен; визуальное
сравнение со старым LiveView на всех viewport и ручное принятие пользователя ещё нужны.

## Этапы и критерии завершения

### 1. Укрепить первый React-срез — следующий шаг

- [ ] Перевести interaction-тесты на `.tsx`; entry point, экран и transport API уже TypeScript.
...
- [ ] Расширить ESLint с типовой информацией, Hooks, accessibility,
...
- [ ] Сохранить пары old/new для одинаковых состояний на телефоне, планшете
...
3. До нового UI-среза завершить этап 1. Ближайший шаг: перенести
   `profiles.test.jsx` на строгий TSX без подавления ошибок и добавить проверку
   границ/циклов импортов; после этого выполнить browser- и visual-сравнение.
   Не менять продуктовый дизайн.
