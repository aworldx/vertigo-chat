...
func (s *Store) Save(ctx context.Context, input domain.Media, field string) (domain.Media, error) {
	key := "gallery/" + field + "/" + hash(input.Bytes) + "." + map[string]string{"image/jpeg": "jpg", "image/png": "png", "image/webp": "webp"}[input.ContentType]
	if err := s.putAndVerify(ctx, key, input.Bytes, input.ContentType); err != nil {
		return domain.Media{}, err
	}
	return domain.Media{Key: key, ContentType: input.ContentType}, nil
}
func (s *Store) Load(ctx context.Context, key string) ([]byte, error) {
	uri, err := s.objectURL(key, true)
	if err != nil {
		return nil, err
	}
	...
	data, err := io.ReadAll(io.LimitReader(response.Body, domain.MaxPhotoBytes+1))
	if err != nil {
		return nil, err
	}
	if response.StatusCode != 200 || len(data) > domain.MaxPhotoBytes {
		return nil, domain.ErrUnavailable
	}
	return data, nil
}
...
func (s Store) putAndVerify(ctx context.Context, key string, body []byte, contentType string) error {
	...
	defer func() { _ = response.Body.Close() }()
	actual, err := io.ReadAll(io.LimitReader(response.Body, domain.MaxPhotoBytes+1))
