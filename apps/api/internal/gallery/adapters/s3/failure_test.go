...
func TestStorageFailuresNeverPublishAnObjectReference(t *testing.T) {
	...
	for _, mode := range []string{"put network", "put status", "get network", "get status", "get corrupt", "get read", "redirect", "invalid endpoint", "invalid public"} {
		t.Run(mode, func(t *testing.T) {
			...
			result, err := s.Save(ctx, domain.Media{Bytes: []byte("image"), ContentType: "image/png"}, "image")
...
func TestLoadPropagatesTransportAndReadFailures(t *testing.T) {
	for _, mode := range []string{"network", "read", "invalid"} {
		s, err := New(S3Config{Endpoint: "https://storage.example", Region: "test", Bucket: "bucket", AccessKeyID: "test", SecretAccessKey: "test"})
		if err != nil {
			t.Fatal(err)
		}
		if mode == "invalid" {
		...
		s.client.Transport = failingTransport(func(*http.Request) (*http.Response, error) {
			if mode == "network" {
				return nil, errors.New("offline")
			}
			return &http.Response{StatusCode: 200, Body: failedReader{}}, nil
		})
		if _, err := s.Load(context.Background(), "image"); err == nil {
