# Назначение файла: базовая конфигурация приложения, общая для всех окружений.
# This file is responsible for configuring your application
# and its dependencies with the aid of the Config module.
#
# This configuration file is loaded before any dependency and
# is restricted to this project.

# General application configuration
import Config

config :chat,
  ecto_repos: [Chat.Repo],
  generators: [timestamp_type: :utc_datetime]

config :chat, Chat.MediaShares, ice_servers: [%{urls: "stun:stun.cloudflare.com:3478"}]

config :chat, :metrics_token, "development-metrics-token"

config :chat, Chat.Bot,
  provider: Chat.Bot.OpenAI,
  reply_delay_range_ms: {5_000, 15_000}

config :chat, Chat.Karmik,
  enabled?: true,
  provider: Chat.Karmik.OpenAI,
  review_interval_ms: 180_000

config :chat, Chat.Games.Dictionary, provider: Chat.Games.Wiktionary

config :mime, :types, %{"audio/ogg" => ["ogg"]}

# Configure the endpoint
config :chat, ChatWeb.Endpoint,
  url: [host: "localhost"],
  adapter: Bandit.PhoenixAdapter,
  render_errors: [
    formats: [html: ChatWeb.ErrorHTML, json: ChatWeb.ErrorJSON],
    layout: false
  ],
  pubsub_server: Chat.PubSub,
  live_view: [signing_salt: "aRerDLdd"]

# Configure LiveView
config :phoenix_live_view,
  # the attribute set on all root tags. Used for Phoenix.LiveView.ColocatedCSS.
  root_tag_attribute: "phx-r"

# Configure esbuild (the version is required)
config :esbuild,
  version: "0.25.4",
  chat: [
    args:
      ~w(js/app.js js/profiles.tsx js/account_login.tsx --bundle --target=es2022 --outdir=../phoenix/priv/static/assets/js --external:/fonts/* --external:/images/* --alias:@=.),
    cd: Path.expand("../../web", __DIR__),
    env: %{"NODE_PATH" => [Path.expand("../deps", __DIR__), Mix.Project.build_path()]}
  ]

# Configure tailwind (the version is required)
config :tailwind,
  version: "4.3.0",
  chat: [
    args: ~w(
      --input=../web/css/app.css
      --output=priv/static/assets/css/app.css
    ),
    cd: Path.expand("..", __DIR__),
    env: %{"NODE_PATH" => [Path.expand("../deps", __DIR__), Mix.Project.build_path()]}
  ]

# Configure Elixir's Logger
config :logger, :default_formatter,
  format: "$time $metadata[$level] $message\n",
  metadata: [:request_id]

# Use Jason for JSON parsing in Phoenix
config :phoenix, :json_library, Jason

config :phoenix, :filter_parameters, [
  "password",
  "token",
  "sdp",
  "candidate",
  "media_chunk"
]

# Import environment specific config. This must remain at the bottom
# of this file so it overrides the configuration defined above.
import_config "#{config_env()}.exs"
